...
spec = do
    ...
    describe "Converting lengths" $ do
        ...
        it "converts metres to nautical miles" $
            nauticalMiles (ofMetres 1000) `shouldBe` 0.5399568034557235
        it "converts kilometres to nautical miles" $
            nauticalMiles (ofKilometres 1000) `shouldBe` 539.9568034557235
        it "converts nautical miles to metres" $ metres (ofNauticalMiles 10.5) `shouldBe` 19446
        it "converts nautical miles to kilometres" $
            kilometres (ofNauticalMiles 10.5) `shouldBe` 19.446
    describe "Adding/Subtracting lengths" $ do
        it "adds lengths" $
            add (ofKilometres 1000) (ofMetres 1000) `lengthShouldBe` ofMetres 1001000
        it "subtracts lengths" $
            sub (ofMetres 1000) (ofNauticalMiles 10.5) `lengthShouldBe` ofMetres (-18446)
