...
spec = do
    describe "Antipode" $ do
        it "returns the antipodal point" $ do
            let p = spherical84 (angularPos (readLatLong "484137N0061105E") 15000)
            let e = spherical84 (angularPos (latLongDecimal (-48.6936111) (-173.8152777)) 15000)
            antipode p `shouldBe` e
        it "returns the south pole when called with the north pole" $
            antipode (spherical84 (nvectorToLatLong northPole)) `shouldBe`
            spherical84 (latLongDecimal (-90.0) (-180.0))
        it "returns the north pole when called with the south pole" $
            antipode (spherical84 (nvectorToLatLong southPole)) `shouldBe`
            spherical84 (latLongDecimal 90.0 (-180.0))
    describe "Surface Distance" $ do
        it "returns 0 if both points are equal" $ do
            let p = spherical84 (readLatLong "500359N1795959W")
            surfaceDistance p p `shouldBe` metres 0.0
        it "returns the distance between 2 points" $ do
            ...
            surfaceDistance p1 p2 `shouldBe` metres 968854.868
        it "handles singularity at the pole" $ do
            let p1 = spherical84 northPole
            let p2 = spherical84 southPole
            surfaceDistance p1 p2 `shouldBe` kilometres 20015.114351
        it "handles the discontinuity at the Date Line" $ do
            let p1 = spherical84 (readLatLong "500359N1795959W")
            let p2 = spherical84 (readLatLong "500359N1795959E")
            surfaceDistance p1 p2 `shouldBe` metres 39.66
    describe "Initial bearing" $ do
        ...
        it "returns the initial bearing in compass angle" $ do
            let p1 = spherical84 (angularPos (readLatLong "500359N0054253W") 12000)
            let p2 = spherical84 (angularPos (readLatLong "583838N0030412W") 5000)
            initialBearing p1 p2 `shouldBe` decimalDegrees 9.1198181
        it "returns the initial bearing in compass angle" $ do
            let p1 = spherical84 (angularPos (readLatLong "583838N0030412W") 12000)
            let p2 = spherical84 (angularPos (readLatLong "500359N0054253W") 5000)
            initialBearing p1 p2 `shouldBe` decimalDegrees 191.2752013
    describe "Interpolate" $ do
        let p1 = spherical84 (readLatLong "44N044E")
        let p2 = spherical84 (readLatLong "46N046E")
        it "fails if f < 0.0" $
        ...
        it "returns the interpolated position" $ do
            let p3 = spherical84 (angularPos (readLatLong "53°28'46''N 2°14'43''W") 10000)
            let p4 = spherical84 (angularPos (readLatLong "55°36'21''N 13°02'09''E") 20000)
            interpolate p3 p4 0.5 `shouldBe`
                spherical84 (angularPos (latLongDecimal 54.7835574 5.1949856) 15000)
    describe "insideSurface" $ do
        let p1 = spherical84 (latLongDecimal 45 1)
        let p2 = spherical84 (latLongDecimal 45 2)
        let p3 = spherical84 (latLongDecimal 46 1)
        let p4 = spherical84 (latLongDecimal 46 2)
        let p5 = spherical84 (latLongDecimal 45.1 1.1)
        ...
        it "handles concave polygons" $ do
            let malmo = spherical84 (latLongDecimal 55.6050 13.0038)
            let ystad = spherical84 (latLongDecimal 55.4295 13.82)
            let lund = spherical84 (latLongDecimal 55.7047 13.1910)
            let helsingborg = spherical84 (latLongDecimal 56.0465 12.6945)
            let kristianstad = spherical84 (latLongDecimal 56.0294 14.1567)
            let polygon = [malmo, ystad, kristianstad, helsingborg, lund]
            let hoor = spherical84 (latLongDecimal 55.9295 13.5297)
            let hassleholm = spherical84 (latLongDecimal 56.1589 13.7668)
    ...
    describe "Final bearing" $ do
        it "returns the 180.0 if both point are the same" $ do
            let p = spherical84 (readLatLong "500359N0054253W")
        ...
        it "returns the final bearing in compass angle" $ do
            let p1 = spherical84 (readLatLong "583838N0030412W")
            let p2 = spherical84 (readLatLong "500359N0054253W")
            finalBearing p1 p2 `shouldBe` decimalDegrees 189.1198181
        it "returns the final bearing in compass angle" $ do
            let p1 = spherical84 (readLatLong "535941S0255915W")
            let p2 = spherical84 (readLatLong "54N154E")
            finalBearing p1 p2 `shouldBe` decimalDegrees 125.6839436
    describe "Mean" $ do
        it "returns Nothing if no point is given" $ (mean [] :: Maybe (GeoPos NVector Length)) `shouldBe` Nothing
        it "returns the unique given point" $ do
            let p = spherical84 (readLatLong "500359N0054253W")
            mean [p] `shouldBe` Just p
        it "returns the geographical mean" $ do
            let p1 = spherical84 (angularPos (readLatLong "500359N0054253W") 15000)
            let p2 = spherical84 (angularPos (readLatLong "583838N0030412W") 25000)
            let e = spherical84 (angularPos (latLongDecimal 54.3622869 (-4.5306725)) 0)
            mean [p1, p2] `shouldBe` Just e
        it "returns Nothing if list contains antipodal points" $ do
            let points =
                    [ spherical84 (latLongDecimal 45 1)
                    , spherical84 (latLongDecimal 45 2)
                    , spherical84 (latLongDecimal 46 2)
                    , spherical84 (latLongDecimal 46 1)
                    , antipode (spherical84 (latLongDecimal 45 2))
                    ]
