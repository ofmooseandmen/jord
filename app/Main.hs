...
showV (Llh g) = "geographic position: " ++ show g
showV (Llhs gs) = "geographic position(S): " ++ intercalate "; " (map show gs)
showV (LlDec ll) = "latitude, longitude (dd): " ++ show (fst ll) ++ ", " ++ show (snd ll)
showV (LlsDec lls) =
    "latitude(s), longitude(s) (dd): " ++
    intercalate "; " (map (\ll -> show (fst ll) ++ ", " ++ show (snd ll)) lls)
showV (Nvh v) = "n-vector: " ++ show v
showV (Nvhs vs) = "n-vector(s): " ++ intercalate "; " (map show vs)
