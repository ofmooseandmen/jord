...
spec = do
    describe "antipode" $ do
        it "returns the antipodal position" $ do
            let p = latLongHeightPos 45 154 (metres 15000) S84
            let e = latLongHeightPos (-45) (-26) (metres 15000) S84
            antipode p `shouldBe` e
        it "returns the south pole when called with the north pole" $
            antipode (northPole S84) `shouldBe` latLongPos (-90) (-180) S84
        it "returns the north pole when called with the south pole" $
            antipode (southPole S84) `shouldBe` latLongPos (90) (-180) S84
    describe "destination" $ do
        it "return the given position if distance is 0 meter" $ do
            let p0 = latLongPos 53.320556 (-1.729722) S84
        ...
        it "return the position along great-circle at distance and bearing (llh)" $ do
            let p0 = latLongHeightPos 53.320556 (-1.729722) (metres 15000.0) S84
            let p1 = latLongHeightPos 53.1882697 0.1332747 (metres 15000.0) S84
    ...
    describe "finalBearing" $ do
        ...
        it "returns 0° if both positions have the same longitude (going north)" $ do
            let p1 = latLongHeightPos 50.066389 (-5.714722) (metres 12000) S84
            let p2 = latLongHeightPos 58.643889 (-5.714722) (metres 5000) S84
            finalBearing p1 p2 `shouldBe` Just (decimalDegrees 0)
        it "returns 180° if both positions have the same longitude (going south)" $ do
            let p1 = latLongHeightPos 58.643889 (-5.714722) (metres 5000) S84
            let p2 = latLongHeightPos 50.066389 (-5.714722) (metres 12000) S84
            finalBearing p1 p2 `shouldBe` Just (decimalDegrees 180)
        it "returns 90° at the equator going east" $ do
            let p1 = latLongHeightPos 0 0 (metres 12000) S84
            let p2 = latLongHeightPos 0 1 (metres 5000) S84
            finalBearing p1 p2 `shouldBe` Just (decimalDegrees 90)
        it "returns 270° at the equator going west" $ do
            let p1 = latLongHeightPos 0 1 (metres 12000) S84
            let p2 = latLongHeightPos 0 0 (metres 5000) S84
            finalBearing p1 p2 `shouldBe` Just (decimalDegrees 270)
        it "returns the final bearing in compass angle" $ do
            let p1 = latLongPos 50.066389 (-5.714722) S84
            let p2 = latLongPos 58.643889 (-3.07) S84
            finalBearing p1 p2 `shouldBe` Just (decimalDegrees 11.2752)
        it "returns the final bearing in compass angle" $ do
            let p1 = latLongPos 58.643889 (-3.07) S84
            let p2 = latLongPos 50.066389 (-5.714722) S84
            finalBearing p1 p2 `shouldBe` Just (decimalDegrees 189.1198172)
        it "returns the final bearing in compass angle" $ do
            let p1 = latLongPos (-53.994722) (-25.9875) S84
            let p2 = latLongPos 54 154 S84
    ...
    describe "initialBearing" $ do
        it "returns Nothing if both positions are the same" $ do
            let p = latLongPos 50.066389 (-179.999722) S84
            initialBearing p p `shouldBe` Nothing
        it "returns 0° if both positions have the same longitude (going north)" $ do
            let p1 = latLongHeightPos 50.066389 (-5.714722) (metres 12000) S84
            let p2 = latLongHeightPos 58.643889 (-5.714722) (metres 12000) S84
            initialBearing p1 p2 `shouldBe` Just (decimalDegrees 0)
        it "returns 180° if both positions have the same longitude (going south)" $ do
            let p1 = latLongHeightPos 58.643889 (-5.714722) (metres 12000) S84
            let p2 = latLongHeightPos 50.066389 (-5.714722) (metres 12000) S84
        ...
        it "returns 0° at the prime meridian going north" $ do
            let p1 = latLongPos 50 0 S84
            let p2 = latLongPos 58 0 S84
            initialBearing p1 p2 `shouldBe` Just (decimalDegrees 0)
        it "returns 180° at the prime meridian going south" $ do
            let p1 = latLongPos 58 0 S84
            let p2 = latLongPos 50 0 S84
            initialBearing p1 p2 `shouldBe` Just (decimalDegrees 180)
        it "returns 0° at the date line going north" $ do
            let p1 = latLongPos 50 180 S84
            let p2 = latLongPos 58 180 S84
            initialBearing p1 p2 `shouldBe` Just (decimalDegrees 0)
        it "returns 180° at the date line going south" $ do
            let p1 = latLongPos 58 180 S84
            let p2 = latLongPos 50 180 S84
            initialBearing p1 p2 `shouldBe` Just (decimalDegrees 180)
        ...
        it "returns the initial bearing in compass angle" $ do
            let p1 = latLongHeightPos 50.066389 (-5.714722) (metres 12000) S84
            let p2 = latLongHeightPos 58.643889 (-3.07) (metres 5000) S84
            initialBearing p1 p2 `shouldBe` Just (decimalDegrees 9.1198172)
        it "returns the initial bearing in compass angle" $ do
            let p1 = latLongHeightPos 58.643889 (-3.07) (metres 12000) S84
            let p2 = latLongHeightPos 50.066389 (-5.714722) (metres 5000) S84
    ...
    describe "surfaceDistance" $ do
        it "returns 0 if both points are equal" $ do
            let p = latLongHeightPos 50.066389 (-5.714722) (metres 15000.0) S84
        ...
        it "handles the discontinuity at the Date Line" $ do
            let p1 = latLongPos 50.066389 (-179.999722) S84
            let p2 = latLongPos 50.066389 179.999722 S84
