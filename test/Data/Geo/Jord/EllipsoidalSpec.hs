...
spec = do
      describe "target" $ do
          ...
          it "computes the target point from p0 and NED vector" $ do
              ...
              target p0 d wgs84 `shouldBe` decimalLatLong 48.8866688 2.37472111
      describe "delta" $ do
          it "computes NED Vector between LatLong positions" $ do
              ...
              d `shouldBe` nedMetres (-86125.88) (-78900.087) 1069.198
          it "computes NED Vector between angular positions" $ do
              ...
              let d = delta p1 p2 wgs84
              d `shouldBe` nedMetres (-86125.88) (-78900.087) 1069.198
      describe "delta and target consistency" $ do
