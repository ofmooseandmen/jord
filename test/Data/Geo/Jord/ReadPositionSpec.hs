...
spec = do
    describe "Reading valid DMS text" $ do
        it "reads 553621N0130002E" $
            ...
            Just (latLongPos 55.6058333 13.0005555 WGS84)
        it "reads 55°36'21''N 013°00'02''E" $
            readPosition "55°36'21''N 013°00'02''E" WGS84 `shouldBe`
            Just (latLongPos 55.6058333 13.0005555 WGS84)
        it "reads 5536N01300E" $
            readPosition "5536N01300E" WGS84 `shouldBe` Just (latLongPos 55.6 13.0 WGS84)
        it "reads 55N013E" $
            readPosition "55N013E" WGS84 `shouldBe` Just (latLongPos 55.0 13.0 WGS84)
        it "reads 011659S0364900E" $
            readPosition "011659S0364900E" WGS84 `shouldBe`
            Just (latLongPos (-1.2830555) 36.8166666 WGS84)
        it "reads 0116S03649E" $
            readPosition "0116S03649E" WGS84 `shouldBe`
            Just (latLongPos (-1.2666666) 36.8166666 WGS84)
        it "reads 1°16'S,36°49'E" $
            readPosition "1°16'S,36°49'E" WGS84 `shouldBe`
            Just (latLongPos (-1.2666666) 36.8166666 WGS84)
        it "reads 01S036E" $
            readPosition "01S036E" WGS84 `shouldBe` Just (latLongPos (-1.0) 36.0 WGS84)
        it "reads 473622N1221955W" $
            readPosition "473622N1221955W" WGS84 `shouldBe`
            Just (latLongPos 47.6061111 (-122.3319444) WGS84)
        it "reads 4736N12219W" $
            readPosition "4736N12219W" WGS84 `shouldBe`
            Just (latLongPos 47.6 (-122.3166666) WGS84)
        it "reads 47N122W" $
            readPosition "47N122W" WGS84 `shouldBe` Just (latLongPos 47.0 (-122.0) WGS84)
        it "reads 47°N 122°W" $
            readPosition "47°N 122°W" WGS84 `shouldBe` Just (latLongPos 47.0 (-122.0) WGS84)
        it "reads 544807S0681811W" $
            readPosition "544807S0681811W" WGS84 `shouldBe`
            Just (latLongPos (-54.8019444) (-68.3030555) WGS84)
        it "reads 5448S06818W" $
            readPosition "5448S06818W" WGS84 `shouldBe`
            Just (latLongPos (-54.8) (-68.3) WGS84)
        it "reads 54S068W" $
            readPosition "54S068W" WGS84 `shouldBe` Just (latLongPos (-54.0) (-68.0) WGS84)
        it "reads 54S360E (Mars)" $
            readPosition "54S360E" Mars `shouldBe` Just (latLongPos (-54.0) 360 Mars)
